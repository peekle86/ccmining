            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('from') ? 'is-invalid' : '' }}" name="from_id" id="from_id" required>
                    @foreach($froms as $id => $entry)
                        <option value="{{ $id }}" {{ (old('from_id') ? old('from_id') : $message->from->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('from'))
                    <div class="invalid-feedback">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('to') ? 'is-invalid' : '' }}" name="to_id" id="to_id">
                    @foreach($tos as $id => $entry)
                        <option value="{{ $id }}" {{ (old('to_id') ? old('to_id') : $message->to->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('to'))
                    <div class="invalid-feedback">
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\Message::READ_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('read') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="read_{{ $key }}" name="read" value="{{ $key }}" {{ old('read', $message->read) === (string) $key ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('chat') ? 'is-invalid' : '' }}" name="chat_id" id="chat_id" required>
                    @foreach($chats as $id => $entry)
                        <option value="{{ $id }}" {{ (old('chat_id') ? old('chat_id') : $message->chat->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('chat'))
                    <div class="invalid-feedback">
