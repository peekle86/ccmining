    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.users.update", [$user->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('ref') ? 'is-invalid' : '' }}" type="text" name="ref" id="ref" value="{{ old('ref', $user->ref) }}" required>
                @if($errors->has('ref'))
                    <div class="invalid-feedback">
                        {{ $errors->first('ref') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('parent') ? 'is-invalid' : '' }}" name="parent_id" id="parent_id">
                    @foreach($parents as $id => $entry)
                        <option value="{{ $id }}" {{ (old('parent_id') ? old('parent_id') : $user->parent->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('ref_percent') ? 'is-invalid' : '' }}" type="text" name="ref_percent" id="ref_percent" value="{{ old('ref_percent', $user->ref_percent) }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('wallet') ? 'is-invalid' : '' }}" name="wallet_id" id="wallet_id">
                    @foreach($wallets as $id => $entry)
                        <option value="{{ $id }}" {{ (old('wallet_id') ? old('wallet_id') : $user->wallet->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
