    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.categories.update", [$category->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('desc') ? 'is-invalid' : '' }}" type="text" name="desc" id="desc" value="{{ old('desc', $category->desc) }}">
                @if($errors->has('desc'))
                    <div class="invalid-feedback">
                        {{ $errors->first('desc') }}
                    </div>
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('show_menu') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="show_menu" id="show_menu" value="1" {{ $category->show_menu || old('show_menu', 0) === 1 ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('template') ? 'is-invalid' : '' }}" name="template" id="template">
                    ...
                    @foreach(App\Models\Category::TEMPLATE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('template', $category->template) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('language') ? 'is-invalid' : '' }}" name="language_id" id="language_id">
                    @foreach($languages as $id => $entry)
                        <option value="{{ $id }}" {{ (old('language_id') ? old('language_id') : $category->language->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
