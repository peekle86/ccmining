    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.contracts.update", [$contract->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('amount') ? 'is-invalid' : '' }}" type="number" name="amount" id="amount" value="{{ old('amount', $contract->amount) }}" step="0.01" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('hardware') ? 'is-invalid' : '' }}" name="hardware_id" id="hardware_id" required>
                    @foreach($hardware as $id => $entry)
                        <option value="{{ $id }}" {{ (old('hardware_id') ? old('hardware_id') : $contract->hardware->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('period') ? 'is-invalid' : '' }}" name="period_id" id="period_id" required>
                    @foreach($periods as $id => $entry)
                        <option value="{{ $id }}" {{ (old('period_id') ? old('period_id') : $contract->period->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control datetime {{ $errors->has('ended_at') ? 'is-invalid' : '' }}" type="text" name="ended_at" id="ended_at" value="{{ old('ended_at', $contract->ended_at) }}">
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\Contract::ACTIVE_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('active') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="active_{{ $key }}" name="active" value="{{ $key }}" {{ old('active', $contract->active) === (string) $key ? 'checked' : '' }} required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('percent') ? 'is-invalid' : '' }}" type="number" name="percent" id="percent" value="{{ old('percent', $contract->percent) }}" step="0.01">
