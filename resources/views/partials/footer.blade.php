...
<div class="items-center justify-center modal text-black hidden" id="calc_modal"
     onclick="toggleModal('calc_modal');return false;">
    <div class="w-600 bg-white rounded-xl max-w-4xl py-12 px-6 space-y-5" onclick="event.stopPropagation();">
        ...
        <div>
            <input oninput="changeCalc(this)" type="range" id="power_range" min="2500"
                   max="{{ $setting->max_th * 1000 }}" value="{{ $setting->max_th * 1000 / 4 }}"
        ...
        <div class="flex space-x-2">
            <div class="flex-1">
                ...
                <input oninput="changeCalc(this)" type="number" step="10" id="amount" min="250"
                       max="{{ $setting->price_th * $setting->max_th * 1000 }}" value="250"
                       class="w-full border p-3 bg-gray-100 rounded">
            </div>
            <div class="flex-1 hidden">
                <div>{{ __('welcome.pow_in') }} GH/s</div>
                <input oninput="changeCalc(this)" type="number" step="10" max="{{ $setting->max_th * 1000 }}" id="power"
                       value="" class="w-full border p-3 bg-gray-100 rounded">
            </div>
        </div>
...
{{-- <script>
    let inputs_with_limit = document.querySelectorAll('input[type="number"][max]');
</script> --}}
